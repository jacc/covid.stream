    ...
    def _updateEachCountry(self, new_data):
        for item in new_data['data']:
            if self._countryRedis.get(item['Country/Region']):
                load_country = json.loads(self._countryRedis.get(item['Country/Region']))
    ...
    def update_data(self, new_data_update):
        ...
        if current_data:
            ...
            if read_current_data['apiInformation'].get("lastUpdated", False) == False:
                ...
                self._redis.rename(
                    "latestFigures",
                    read_current_data['apiInformation']['lastUpdated']
                )
                logger.info(f"Moved figures for {read_current_data['apiInformation']['lastUpdated']} to history.")
        
        self._redis.set(
            "latestFigures",
            json.dumps(new_data_update)
        )
